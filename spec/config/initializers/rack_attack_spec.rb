...
describe Rack::Attack do
  ...
  describe 'Blocklist' do
    let(:limit) { 25 }
    context 'When the number of requests by IP address is lower than the limit' do
    ...
    context 'When the number of requests by IP address is higher than the limit' do
  ...
  describe 'Throttle' do
    let(:limit) { 5 }
    context 'When the number of requests to user sign in by IP address is lower than the limit' do
      it 'does not change the request status' do
    ...
    context 'When the number of requests to user sign in by IP address is higher than the limit' do
      it 'changes the request status to 503' do
        (limit * 2).times do |iteration_number|
          post '/api/v1/users/sign_in', {
    ...
    context 'When the number of requests to admin login by IP address is lower than the limit' do
      it 'does not change the request status' do
        limit.times do |iteration_number|
    ...
    context 'When the number of requests to admin login by IP address is higher than the limit' do
      it 'changes the request status to 503' do
        (limit * 2).times do |iteration_number|
